...
const Header = ({ user, handleLogout }) => {
  ...
  const handleSidebarOutside = useCallback((e) => {
  ...
  }, [isSidebarOpen, handleSidebarOutside]);
  ...
  return (
    <header className='w-11/12 flex items-center justify-between py-2'>
      <div className='w-full md:w-full flex justify-center' onClick={handleLogoClick}>
        <a className="flex items-center tracking-wide no-underline hover:no-underline font-bold text-gray-800 text-2xl" href="#">
          <svg className="fill-current text-gray-800 mr-2" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24">
      ...
      {/* Desktop nav */}
      <div className='absolute top-4 right-20 md:flex hidden flex-row justify-center items-center'>
        <Link to='/wishlist'>
          <svg className="h-7 mx-2 w-7 fill-current hover:text-black" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          ...
          {clicked && (
            <div ref={modalRef} className='absolute z-50 top-12 right-0 group px-2 bg-[#f9fafe] border border-gray-200 shadow-md rounded-md w-44 h-82 text-[#374151]'>
              <div className='w-full flex flex-row items-center justify-center py-3 px-2 border-b border-gray-200'>
                ...
                <span className='flex w-3/5 text-m font-bold items-start'>{user?.fname ?? 'Guest User'}</span>
              </div>
      ...
      {/* Mobile sidebar */}
      {isSidebarOpen && (
        <div ref={sidebarRef} className={`top-0 right-0 z-50 fixed sm:w-60 w-52 h-full bg-white shadow-lg transition-transform duration-300 ease-in-out transform ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className='fixed top-3 right-3 cursor-pointer' onClick={toggleSidebar}>
