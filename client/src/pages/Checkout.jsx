...
import { Header, Footer, CheckoutCard, Alerts } from '../components';
import { Link, useNavigate } from 'react-router-dom';
import axios from "axios";
import { CheckoutSkeleton } from '../components/SkeletonAnimations';
...
const Checkout = () => {
  ...
  const [currentAddress, setCurrentAddress] = useState({
    fname: '', lname: '', phone: '', area1: '', area2: '', city: '', state: '', pincode: ''
  });
  ...
  const setup = async () => {
    setSetupDone(false);
    try {
      const [addressRes, checkoutRes, profileRes] = await Promise.all([
        axios.post(`https://${server}/api/user/address/get`, {}, { withCredentials: true }),
        axios.post(`https://${server}/api/user/checkout/get`, {}, { withCredentials: true }),
        axios.post(`https://${server}/api/user/profile/get`, {}, { withCredentials: true }),
      ]);

      if (checkoutRes.data.success) {
        setCheckout(checkoutRes.data.data);
        const cartItems = checkoutRes.data.data.cartItems;
  ...
  const continueToPayment = async () => {
    try {
      ...
      if (response.data.success) navigate('/cart/checkout/payment');
    } catch (err) {
      handleAuthError(err);
    }
  };
  ...
  return (
    <div className='w-full flex flex-col items-center justify-center'>
      ...
      {setupDone === false ? <CheckoutSkeleton /> : (
              ...
              <div className='flex flex-col w-full pt-2 items-center justify-center'>
                <div className="flex w-full flex-col justify-center border md:border-y md:border-x-0 sm:px-4">
                  {checkout?.cartItems?.map((cartItem, ci) => (
                    <div key={ci}>
                      <CheckoutCard ci={ci} cartItem={cartItem} productItem={products[ci]} handleAlert={addAlert} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <table className='w-full ml-4'>
                <tbody>
                  <tr>
                    ...
                    <td className='w-3/12'>Rs. {checkout?.totalAmount}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td className='w-3/12 font-medium pt-3'>Rs. {checkout?.totalAmount && checkout.totalAmount + 30}</td>
                  </tr>
                </tbody>
              </table>
