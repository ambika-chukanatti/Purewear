...
const ForgotPassword = () => {
  ...
  const handlePassword = (e) => setPassword(prev => ({ ...prev, [e.target.name]: e.target.value }));

  const handleSendOTP = async () => {
    setLoading(true);
    try {
      const response = await axios.post(`https://${server}/api/auth/password/forgot`, { email });
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      const response = await axios.post(
        `https://${server}/api/auth/password/verify`,
        { otp },
  ...
  const handleNewPassword = async () => {
    if (password.password1 !== password.password2) {
      addAlert('error', "Passwords don't match");
      return;
    }
    setLoading(true);
    try {
      const response = await axios.post(
        `https://${server}/api/auth/password/reset`,
  ...
  return (
    <div className='relative w-full h-screen flex flex-col md:flex-row'>
      ...
      <div className="absolute top-8 left-8 flex items-center font-bold text-gray-800 text-2xl mb-4">
