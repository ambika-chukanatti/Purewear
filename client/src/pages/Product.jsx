...
const Product = () => {
  ...
  const fetchProduct = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`https://${server}/api/product/get/${pid}`);
      if (res.status === 200) {
  ...
  const setup = async () => {
    ...
    try {
      ...
      const res3 = await axios.post(`https://${server}/api/user/cart/get`, {}, { withCredentials: true });
  ...
  const handleLeftClick = () => {
    if (imgIndex >= product.imageUrls.length - 1) {
      setImgIndex(0);
    } else {
  ...
  const handleRightClick = () => {
    if (imgIndex <= 0) {
      setImgIndex(product.imageUrls.length - 1);
    } else {
      setImgIndex(prev => prev - 1);
    }
  };
  ...
  const handleChange = (e) => {
    if (e.target.name === 'size') {
      setCartItem(prev => ({ ...prev, [e.target.name]: e.target.value }));
  ...
  const addToCart = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        addAlert('success', 'Added to cart');
      }
    } catch (err) {
      handleAuthError(err);
    } finally {
            ...
            <div className="flex flex-col items-center lg:items-start lg:col-span-2 lg:row-span-2 lg:row-end-2 px-4">
              ...
              <div className="mt-3 flex select-none flex-wrap items-center gap-1">
                {product?.colors?.map((color, index) => (
                  <label key={color.url}>
                    ...
                    <img className='peer-checked:border border-black mx-1 w-10 h-10 p-1 cursor-pointer' src={color.url} alt="" />
                  </label>
                ))}
              </div>
              ...
              <div className="mt-10 flex flex-row items-center justify-center border-t border-b py-4 sm:flex-row">
                <div className='w-full flex flex-row items-center justify-start'>
                  <button type="button" className="inline-flex items-center justify-center rounded-md border-2 mr-8 border-transparent bg-gray-900 bg-none px-12 py-3 text-center text-base font-bold text-white transition-all duration-200 ease-in-out focus:shadow hover:bg-gray-800" onClick={addToCart}>
